...
const HOME_MODULES = ['music', 'pomodoro', 'cursor', 'recorder', 'windows', 'mirror', 'note', 'commands'];
...
test('note search matches titles and full content without changing archive order', () => {
  const notes = normalizeNoteArchive([
    { id: 'one', title: 'Pome Panel 设计', titleSource: 'model', content: '正文没有产品英文名', createdAt: 100, updatedAt: 300 },
    { id: 'two', content: '会议备忘\n下周交付录制功能', createdAt: 200, updatedAt: 200 },
  ]);
  assert.deepEqual(filterNotes(notes, 'pome').map((note) => note.id), ['one']);
...
test('home widget packing fills all four rows even when logical order would fragment the grid', () => {
  const order = ['recorder', 'windows', 'commands', 'mirror', 'music', 'note', 'pomodoro', 'cursor'];
  const sizes = {
    recorder: 'small',
    windows: 'large',
    commands: 'mini',
...
test('homepage visibility refuses to hide the final visible module', () => {
  const almostAllHidden = HOME_MODULES.slice(0, -1);
  assert.deepEqual(
    updateHomeModuleVisibility(almostAllHidden, HOME_MODULES, HOME_MODULES.at(-1), false),
...
test('cursor usage module packs into the full eight-module homepage', () => {
  const order = ['music', 'pomodoro', 'cursor', 'windows', 'recorder', 'mirror', 'note', 'commands'];
  const sizes = {
    music: 'small', pomodoro: 'mini', cursor: 'small', windows: 'large',
