...
const isolatedUserData = process.env.TODO_TEST_USER_DATA || fs.mkdtempSync(path.join(os.tmpdir(), 'pome-panel-electron-test-'));
  ...
  try {
    ...
    const persistenceAndRecorderAudit = await window.webContents.executeJavaScript(`
      (() => {
        ...
        ['music', 'pomodoro', 'cursor', 'recorder', 'windows', 'note'].forEach((id) => {
