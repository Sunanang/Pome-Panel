...
- 共享协议在 `packages/sync-protocol`。当前 **P0 只同步待办**。笔记、链接、常用指令、首页布局和剪贴板历史已经作为集合名写在协议里，尚未接入推送与拉取。
- 录音文件、密钥和 API 配置仍然只留在本机。它们不是已经上线的同步内容。
...
- **NAS 同步是附加项**：飞牛伴侣用来在多台设备之间同步待办。它不取代本机存储，也不是一套云端工作台。明文 HTTP 默认关闭，回环地址除外。
