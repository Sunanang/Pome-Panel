...
- 产品版本改为 **0.9.5**。关于窗口显示 Pome Panel、开发者 Lando，以及当前版本；「查看 GitHub」打开 https://github.com/Sunanang/Pome-Panel。
...
- 飞牛包 **0.9.5**。恢复简单启动，修复「执行应用启动脚本失败」。`start.sh` 不再因为没配端口退出；设备端口按环境变量、已保存文件、系统分配的顺序绑定，端口被占用时改绑系统分配的端口。应用中心「发布者」仍是 Lando，入口名仍是 `com.pomepanel.sync.main`，`install_dep_apps=nodejs_v22`。启动 PATH 仍优先包含 Node.js v24 与 v22。生命周期脚本仍用系统自带的 bash。`config/resource` 仍是 data-share。
...
- 从 0.9.x 升级到 0.9.5 后即可按简单启动启用。此前 1.1.x 升到 0.9.x 仍可能被飞牛当成降级而拒绝覆盖安装。
