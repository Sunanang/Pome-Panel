...
- 设备同步端口写入数据目录（`device-port`，可用 `FNOS_DEVICE_PORT_FILE` 改路径），下次启动优先绑定上次的端口。端口被占用时改用系统分配的新端口并改写记录。设备页顶部单独显示当前监听端口，并说明 FRP 本地目标是 `127.0.0.1:此端口`，公网端口可自定；Mac 同步地址仍用 `http://公网IP:公网端口`。端口未开启或读不到时直接写明。
