...
- 产品版本改为 **0.9.1**。关于窗口显示 Pome Panel、开发者 Lando，以及当前版本；「查看 GitHub」打开 https://github.com/Sunanang/Pome-Panel。
...
- 飞牛包 **0.9.1**。应用中心「发布者」读取 manifest 的 `distributor`，现为 Lando；`maintainer` 仍是 Lando。应用标识仍是 `com.pomepanel.sync`，入口仍是 `/app/pome-panel`。`desktop_uidir=ui`，对应包内 `app/ui`，入口名 `com.pomepanel.sync.main`。
- 安装、升级和应用设置向导的端口校验改为 1 到 65535，不再把上限写成 5。安装、配置和升级回调会把 `wizard_port`（以及常见大写别名）写到 `TRIM_PKGETC/device-port` 和数据目录。启动时找不到端口或 Node.js，会把中文原因写入 `TRIM_TEMP_LOGFILE`，不会改用随机端口，也不会写死 5001。
- 从已安装的 0.9.0 升级时，升级向导会再要一次端口。若应用中心不允许在启动失败后改设置，需要卸掉 0.9.0 再装 0.9.1。此前 1.1.x 升到 0.9.x 仍可能被飞牛当成降级而拒绝覆盖安装。
- 安装向导和应用设置要求用户填写设备同步端口（`wizard_port`）。启动只绑定这个端口；没填或被占用就启动失败，不再随机换端口。设备页显示该固定端口，FRP 本地目标是 `127.0.0.1:此端口`。
