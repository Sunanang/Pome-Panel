...
- 设置里的 NAS 同步按「连接状态 / Endpoint / 配对 / 已配对设备」拆成与其他设置卡同一套样式的模块，主操作（添加 endpoint、配对）更醒目，备份与吊销收进次要按钮。
- 修复「绑定本机」在未填写地址、未点配对时也会显示绿色「连接成功」的问题。Endpoint 测试、重试和备份结果留在对应卡片，配对提示只显示配对、重新认证和迁移。
- 修复未绑定时点「重试」把原始错误码 `not_bound` 显示出来的问题，改为「请先完成配对再同步」。其它同步错误码也不再原样出现在设置提示里。
