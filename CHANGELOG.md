...
- 设备同步端口写入数据目录（`device-port`，可用 `FNOS_DEVICE_PORT_FILE` 改路径），下次启动优先绑定上次的端口。端口被占用时改用系统分配的新端口并改写记录，避免升级或重启后 FRP 仍指向旧端口、公网能连上却没有回应。设备页显示当前的 `127.0.0.1:端口`。桌面同步地址仍填公网 FRP 地址。
