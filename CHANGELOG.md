...
- 产品版本改为 **0.9.0**。关于窗口显示 Pome Panel、开发者 Lando，以及当前版本；「查看 GitHub」打开 https://github.com/Sunanang/Pome-Panel。
...
- 飞牛包版本改为 **0.9.0**（此前已发出的包在 1.1.x）。应用中心名称是「Pome Panel」，开发者是 Lando。应用标识仍是 `com.pomepanel.sync`，入口仍是 `/app/pome-panel`。飞牛若把较低版本当成降级，可能拒绝在 1.1.x 上直接覆盖安装。
