...
- 产品版本改为 **0.9.6**。关于窗口显示 Pome Panel、开发者 Lando，以及当前版本；「查看 GitHub」打开 https://github.com/Sunanang/Pome-Panel。
...
- 飞牛包 **0.9.6**。安装目录不可写，启动不再在 `/var/apps/<应用>/runtime` 下建目录。pid 和日志写到 `TRIM_PKGVAR/runtime`（也可由 `FNOS_RUNTIME_DIR` 指定）。数据目录优先 `FNOS_DATA_DIR`，其次 `TRIM_PKGVAR`，再次 `TRIM_PKGHOME`。网关套接字在设置了 `TRIM_APPDEST` 时仍是 `app.sock`。没配端口时网关仍会启动，设备端口仍按环境变量、已保存文件、系统分配的顺序绑定。发布者仍是 Lando，入口名仍是 `com.pomepanel.sync.main`，`install_dep_apps=nodejs_v22`。
...
- 从 0.9.x 升级到 0.9.6 后，启用不再因为安装目录没有写权限而失败。此前 1.1.x 升到 0.9.x 仍可能被飞牛当成降级而拒绝覆盖安装。
